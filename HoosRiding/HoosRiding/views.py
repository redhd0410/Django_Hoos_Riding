...
def user(request, pk=-1):
    if request.method == 'GET':
        ...
        user = User(first_name=json_data["first_name"], last_name=json_data["last_name"],
...
def vehicle(request, pk=-1):
    if request.method == 'GET':
        ...
        vehicle = Vehicle(driver=user, license_plate=json_data['license_plate'],
                          model=json_data['model'], color=json_data['color'], capacity=json_data['capacity'])
        vehicle.save()
        return JsonResponse(json_data)


def ride(request, pk=-1):
   if request.method == 'GET':
      if(pk == -1):
         return HttpResponse(status=400)
      ...
      json_data = json.loads(str(request.body, encoding='utf-8'))
      user = User.objects.get(pk = json_data['driver'])
      vehicle = Ride(vehicle = json_data['vehicle'], destination = json_data['destination'], start = json_data['start'], depart_time = json_data['depart_time'])
      vehicle.save()
      vehicle.user.add(user)
